...
import { AdditiveBlending, Color, DataTexture, Points, Texture } from 'three'
...
const PlayerParticles: FC<Props> = ({ isMobile }) => {
  const dpr = useThree((s) => s.viewport.dpr)
  const performance = useThree((s) => s.performance).current
  const renderer = useThree((s) => s.gl)

  const particlesCount = useMemo(() => Math.pow(isMobile ? 24 : 48 * performance, 2), [isMobile, performance])
  ...
  useGSAP(() => {
    ...
    const applyDamage = () => {
      if (damageAmount.current.value > 0) return // already animating
  ...
  useLayoutEffect(() => {
    ...
    try {
      ...
      fillTextures(dtPosition, dtVelocity, dtSeed, seeds)
      ...
      if (positionUniforms) {
        ...
        positionUniforms.uDamageAmount = { value: 0.0 }
      }
...
const fillTextures = (
  ...
  for (let k = 0, kl = posArray.length; k < kl; k += 4) {
    ...
    velArray[k + 3] = 1.0 // Life always starts at 1.0 (fully alive)
