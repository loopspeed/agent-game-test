...
void main() {
  // Sample the scattered positions texture (larger sphere)
  vec3 scatteredPos = texture2D(uScatteredPositions, vUv).xyz;
  
  // Sample the sphere positions texture (small sphere)
  vec3 spherePos = texture2D(uSpherePositions, vUv).xyz;
  
  // Mix between sphere and scattered positions based on uScatteredAmount
  // uScatteredAmount = 0: use sphere positions
  // uScatteredAmount = 1: use scattered positions
  vec3 pos = mix(spherePos, scatteredPos, uScatteredAmount);
