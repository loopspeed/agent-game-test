...
export default function Player() {
  ...
  useFrame(() => {
    ...
    if (input.left) targetX = lanesX[Math.max(0, lanesX.indexOf(currentX.current) - 1)];
    if (input.right) targetX = lanesX[Math.min(lanesX.length - 1, lanesX.indexOf(currentX.current) + 1)];
    if (input.up) targetY = lanesY[Math.min(lanesY.length - 1, lanesY.indexOf(currentY.current) + 1)];
    if (input.down) targetY = lanesY[Math.max(0, lanesY.indexOf(currentY.current) - 1)];
  ...
  return (
    <RigidBody type="kinematicPosition" colliders="cuboid" position={[0, 0, 0]} ref={bodyRef as any}>
